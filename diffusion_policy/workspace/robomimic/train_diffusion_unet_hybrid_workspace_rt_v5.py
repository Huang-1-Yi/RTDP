            ...
            while self.epoch < cfg.training.num_epochs:
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # 初始化历史预测缓冲区（用于模拟推理中的迭代）
                        history_pred = None  # [B, horizon, D_act]，初始为None
                        ...
                        import math
                        num_windows = math.ceil(T_obs / n_action_steps)

                        for w in range(num_windows):
                            # 5. 准备单个窗口的数据
                            # 动作窗口：长度为 horizon，从 start_act_idx 开始
                            start_act_idx = w * n_action_steps
                            end_act_idx = start_act_idx + horizon
                            window_actions = actions[:, start_act_idx:end_act_idx, :]  # [B, H, D_act]

                            # 观测窗口：不填充，截断到有效范围
                            start_obs_idx = min(start_act_idx, max(0, T_obs - n_obs_steps))
                            end_obs_idx = start_obs_idx + n_obs_steps
                            window_obs = {k: v[:, start_obs_idx:end_obs_idx] for k, v in obs.items()}

                            # 构建传递给 policy 的 batch
                            # 传入obs、action（部分真实，部分填充）、历史预测（裁剪后的）
                            window_batch = {
                                'obs': window_obs,
                                'action': window_actions,  # 包含真实和填充的动作
                                'history_pred': history_pred  # 上一次预测的裁剪结果
                            }
                            ...
                            # 8. 更新历史预测缓冲区
                            # 裁剪预测动作：移除第一个（已执行），保留后续作为下一次的历史
                            if pred_actions.shape[1] > 1:
                                history_pred = pred_actions[:, 1:, :]  # [B, horizon-1, D_act]
                            else:
                                history_pred = None  # 如果horizon=1，重置
