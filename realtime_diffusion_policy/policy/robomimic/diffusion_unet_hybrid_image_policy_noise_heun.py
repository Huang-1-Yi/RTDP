...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch):
        # 1. 检查batch中是否包含valid_mask（本实现不使用）
        ...
        if self.obs_as_global_cond:
            ...
            # 14. 通过观测编码器提取特征
            nobs_features = self.obs_encoder(this_nobs)
            # 15. 将特征重塑回(B, T, Do)的形状
            nobs_features = nobs_features.reshape(batch_size, horizon, -1)
            # 16. 将动作和观测特征拼接作为条件数据
            cond_data = torch.cat([nactions, nobs_features], dim=-1)
            # 17. 更新轨迹数据为条件数据（用于后续掩码生成）
        ...
        # 22. 自定义加噪过程（替代noise_scheduler.add_noise）
        #     获取对应时间步的alpha_bar值
        alpha_bar_t = self.alpha_bar[timesteps]  # 形状 [bsz]
        # 23. 重塑alpha_bar_t为[bsz, 1, 1]以便广播
        ...
        if pred_type == 'epsilon':
            target = noise  # 目标为添加的噪声
        elif pred_type == 'sample':
            target = trajectory  # 目标为原始轨迹
        else:
