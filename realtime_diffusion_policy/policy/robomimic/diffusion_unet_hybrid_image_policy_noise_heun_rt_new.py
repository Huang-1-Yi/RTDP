...
class SlidingWindowDiffusionPolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch):
        ...
        for window_start in range(0, max_window_start):
            ...
            global_cond = self._prepare_global_condition(window_obs)
            ...
            expanded_global_cond = global_cond.repeat(self.parallel_value, 1)# 扩展 观测 批次维度以匹配并行处理

            # # 打印扩展后的数据形状
            # print(f"Expanded noisy shape: {expanded_noisy.shape}")    # Expanded noisy shape: torch.Size([24, 128, 10])
            # print(f"Expected shape: [{self.parallel_value}, {self.horizon*self.n_action_steps}, {Da}]")   # Expected shape: [24, 128, 10]
        
            # 8. UNet前向传播
            # 使用虚拟的时间步进行初始预测（固定使用最高噪声强度）
            timesteps = torch.full((self.parallel_value,), 
                                self.num_train_timesteps - 1,  # 固定使用最高噪声强度
                                device=device, dtype=torch.long)
            
            # # 由于数据已经是3维的，不需要额外重塑
            # # 直接使用 expanded_noisy 作为模型输入
            # print(f"Model input shape: {expanded_noisy.shape}") # Model input shape: torch.Size([24, 128, 10])
            # print(f"Expected shape: [{self.parallel_value}, {self.horizon*self.n_action_steps}, {Da}]") # Expected shape: [24, 128, 10]
            
            # 调用模型预测噪声
            pred_noise = self.model(expanded_noisy, timesteps, global_cond=expanded_global_cond)
            ...
            loss_per_element = F.mse_loss(pred_noise, 
                                        ...
                                        reduction='none')
            # print(f"Loss per element shape: {loss_per_element.shape}")  # Loss per element shape: [24, 128, 10]
            # 应用位置权重
            ...
            # # 只计算真实部分（排除填充部分）
            # # 计算当前窗口的真实长度
            # start_idx = window_start * self.n_action_steps # 窗口起始位置=窗口起始块*每块时间步数
            # end_idx = (window_start + self.horizon) * self.n_action_steps # 窗口结束位置=(窗口起始块+窗口大小)*每块时间步数
            # real_length = min(end_idx, padded_actions.shape[1]) - start_idx   # 实际可用的动作序列长度（避免超出原始序列长度）=窗口结束位置与填充后动作长度的较小值-窗口起始位置
            
            # # 只保留真实部分（裁剪掉填充部分）
            # if real_length < weighted_loss.shape[1]:
            #     # 裁剪加权损失，只保留真实部分
            #     weighted_loss_real = weighted_loss[:, :real_length, :]
            # else:
            #     weighted_loss_real = weighted_loss
        
            # # 对所有并行样本和位置求平均
            # if weighted_loss_real.numel() > 0:
            #     loss = weighted_loss_real.mean()
            # else:
