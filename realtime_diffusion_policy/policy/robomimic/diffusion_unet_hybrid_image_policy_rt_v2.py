...
class SlidingWindowDiffusionPolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        #    注意：若 obs 中包含非数值辅助键（如 window_info），需要在归一化前剔除。
        obs_in = batch['obs']
        if isinstance(obs_in, dict) and ('window_info' in obs_in):
            obs_main = {k: v for k, v in obs_in.items() if k != 'window_info'}
            # 可选：如需在损失中使用 window_info，可从 obs_in['window_info'] 读取（此处不使用）
        else:
            obs_main = obs_in
        nobs = self.normalizer.normalize(obs_main)
    ...
    def _initialize_inference_buffer(self, obs_dict: Dict[str, torch.Tensor]):
        ...
        # 剔除可能存在的 window_info 等非归一化键
        obs_in = obs_dict
        if isinstance(obs_in, dict) and ('window_info' in obs_in):
            obs_in = {k: v for k, v in obs_in.items() if k != 'window_info'}
