...
class SlidingWindowDiffusionPolicy(BaseImagePolicy):
    ...
    def _prepare_global_condition(self, obs_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        准备全局条件信息
        ...
        返回:
            global_cond: 形状为 [batch_size, global_cond_dim] 的全局条件
        """
        # 从任意观测键获取批次大小(使用更稳健的方式获取批次大小)
        ...
        return obs_features # 无需扩展批次维度，因为模型本身会广播
    ...
    def compute_loss(self, batch):
        ...
        H = self.horizon
        S = self.n_action_steps
        assert T == H * S, f"Window size mismatch: T={T}, H*S={H*S}"

        # 生成噪声水平：从0到H-1
        noise_levels = torch.arange(H, device=nactions.device)  # [H]
        # 扩展噪声水平到每个时间步
        expanded_noise_levels = noise_levels.repeat_interleave(S).view(1, H*S, 1)  # [1, H*S, 1]
        
        # 获取alpha_bar值
        alpha_bar_t = self.alpha_bar[expanded_noise_levels].to(nactions.device)  # [1, H*S, 1]
        
        # 生成随机噪声
        true_noise = torch.randn_like(nactions)  # [B, H*S, D]
        
        # 应用噪声
        noisy_actions = torch.sqrt(alpha_bar_t) * nactions + torch.sqrt(1 - alpha_bar_t) * true_noise
        
        # 第一个块（索引0到S-1）保持无噪声
        noisy_actions[:, :S] = nactions[:, :S]

        # 准备全局条件：编码观测
        this_obs = dict_apply(nobs, lambda x: x.reshape(-1, *x.shape[2:]))
        obs_features = self.obs_encoder(this_obs)
        obs_features = obs_features.reshape(B, -1)  # [B, n_obs_steps * Do]

        # 扩展批次以进行并行处理（如果使用）
        if self.parallel_value > 1:
            noisy_actions = noisy_actions.repeat(self.parallel_value, 1, 1)  # [B*parallel, H*S, D]
            obs_features = obs_features.repeat(self.parallel_value, 1)  # [B*parallel, n_obs_steps*Do]
            true_noise = true_noise.repeat(self.parallel_value, 1, 1)  # [B*parallel, H*S, D]
        else:
            # 确保维度一致
            noisy_actions = noisy_actions  # [B, H*S, D]
            obs_features = obs_features  # [B, n_obs_steps*Do]
            true_noise = true_noise  # [B, H*S, D]

        # 使用虚拟的时间步（最大噪声步长）进行预测
        timesteps = torch.full((noisy_actions.shape[0],), self.num_train_timesteps-1, device=noisy_actions.device, dtype=torch.long)

        # 预测噪声
        pred_noise = self.model(noisy_actions, timesteps, global_cond=obs_features)

        # 计算损失，并加权
        weights = self.window_weights.repeat_interleave(S).view(1, H*S, 1).to(noisy_actions.device)  # [1, H*S, 1]
        # 第一个块的权重为0
        weights[:, :S] = 0

        loss_per_element = F.mse_loss(pred_noise, true_noise, reduction='none')
    ...
    def _initialize_inference_buffer(self, obs_dict: Dict[str, torch.Tensor], 
        ...
        # 确保形状正确
        assert self._inference_buffer.shape == (B, self.horizon, self.n_action_steps, self.action_dim), \
            f"Initial buffer shape {self._inference_buffer.shape} != expected {(B, self.horizon, self.n_action_steps, self.action_dim)}"
    ...
    def _denoise_step(self, pred_noise: torch.Tensor) -> torch.Tensor:
        ...
        # 将去噪后的缓冲区转换回4D格式
        denoised_buffer = denoised_buffer_3d.reshape(B, H, S, D)
        
        # 确保形状正确
        assert denoised_buffer.shape == (B, H, S, D), \
            f"Denoised buffer shape {denoised_buffer.shape} != expected {(B, H, S, D)}"
    ...
    def _slide_window(self):
        ...
        # 确保形状正确
        assert self._inference_buffer.shape == (B, H, S, D), \
            f"After slide window, buffer shape {self._inference_buffer.shape} != expected {(B, H, S, D)}"
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        B_obs = next(iter(obs_dict.values())).shape[0]
        device = self.device

        # 如果是第一次调用，或者批次大小发生变化，重新初始化缓冲区
        if (self._inference_buffer is None or 
            self._inference_buffer.shape[0] != B_obs):
        ...
        this_nobs = dict_apply(nobs, lambda x: x.reshape(-1, *x.shape[2:]))
        nobs_features = self.obs_encoder(this_nobs)
        nobs_features = nobs_features.reshape(B_obs, -1)  # [B_obs, To * Do]
        self._inference_global_cond = nobs_features
        

        # 2. 单次去噪
        # 将缓冲区转换为3D格式
        buffer_3d = self._inference_buffer.reshape(B_obs, -1, self.action_dim)
        
        # 使用虚拟的时间步进行初始预测（固定使用最高噪声强度）
        # timesteps = torch.full((B,), 
        #                     self.num_train_timesteps - 1,  # 固定使用最高噪声强度
        #                     device=device, dtype=torch.long)
        timesteps = torch.full((B_obs,), 
                            self.num_train_timesteps - 1,  # 固定使用最高噪声强度
                            device=device, dtype=torch.long)
        ...
        # 3. 输出位置1的动作 (去噪后变为干净值)
        output_actions = self._inference_buffer[:, 1]  # [B_obs, n_action_steps, D]
        ...
        # 打印当前_inference_buffer的形状
        # print(f"_inference_buffer shape: {self._inference_buffer.shape}")   # _inference_buffer shape: torch.Size([32, 16, 8, 10])
        # print(f"Expected shape: [B, H, S, D] = [{B_obs}, {self.horizon}, {self.n_action_steps}, {self.action_dim}]")    # Expected shape: [B, H, S, D] = [1, 16, 8, 10]
    
        # 将缓冲区展平为三维序列
        buffer_flat = self._inference_buffer.reshape(
            B_obs, self.horizon * self.n_action_steps, self.action_dim
        )

        # 我的策略中，应该用执行部分算损失，而不是预测部分    
        return {
            'action': actions,                                                  # 当前时间步立即执行的动作序列​
            'action_pred': self.normalizer['action'].unnormalize(buffer_flat)   # 模型对整个预测时间范围的动作预测
