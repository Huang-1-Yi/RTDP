...
class RobomimicReplayImageDatasetRT(BaseImageDataset):
    def __init__(self,
        ...
        # 连接并自检 SequenceSamplerRT（不影响现有 __getitem__ 行为）
        try:
            ...
            self.sampler_rt = SequenceSamplerRT(
                ...
                action_key='action',
                equalize_windows_per_episode=True
            )
            self._sampler_rt_self_check()
        except Exception as e:
            print(f"[SamplerRT] 构建/自检失败（跳过，不影响数据集继续工作）：{e}")
    ...
    # ========== SamplerRT 自检 ==========
    def _sampler_rt_self_check(self):
        import numpy as np
        rb = self.replay_buffer
        sel_ids = self.sampler_rt.selected_episode_ids()
        print("[SamplerRT] ===== 自检开始 =====")
        print(f"[SamplerRT] 回放缓冲区 episode 数: {rb.n_episodes}，config.n_demo={getattr(self, 'n_demo', 'NA')}")
        lengths = []
        counts = []
        for ep_id in sel_ids:
            L = self.sampler_rt.episode_length(int(ep_id))
            C = self.sampler_rt.episode_window_count(int(ep_id))
            lengths.append(L)
            counts.append(C)
            print(f"[SamplerRT] ep#{int(ep_id):03d}: length={L}, windows={C}")
        if len(counts) > 0:
            counts = np.asarray(counts)
            max_c = int(np.max(counts))
            eq_total = max_c * len(sel_ids)
            actual = len(self.sampler_rt)
            print(f"[SamplerRT] 窗口数统计：min={int(np.min(counts))}, max={max_c}, unique={np.unique(counts).tolist()}")
            print(f"[SamplerRT] 经过对齐（复制最后一个窗口）后的总窗口数（期望）= {eq_total}")
            print(f"[SamplerRT] 实际 indices 行数 = {actual}")
            if actual != eq_total:
                print("[SamplerRT][警告] 对齐总数与实际 indices 长度不一致，请检查 mask/pad 设置！")
        print("[SamplerRT] ===== 自检结束 =====")
