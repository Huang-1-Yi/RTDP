...
class RobomimicReplayImageDatasetRT(BaseImageDataset):
    ...
    def __init__(self,
        ...
        # 使用 SequenceSamplerRT，避免 key_first_k；由 sampler 内部处理 n_obs_steps
        self.sampler = SequenceSamplerRT(
            ...
            action_key='action',
            equalize_windows_per_episode=False  # 保持与原版长度接近
        )
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = SequenceSamplerRT(
            ...
            obs_keys=list(self.rgb_keys) + list(self.lowdim_keys),
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # when self.n_obs_steps is None this slice takes all; sampler 已经按需裁剪
        T_slice = slice(self.n_obs_steps)

        obs_dict = dict()
        for key in self.rgb_keys:
            # move channel last to channel first
            # T,H,W,C -> T,C,H,W; convert uint8 image to float32
        ...
        # 附带 window_info（若 sampler 提供）
        if 'window_info' in data:
            window_info_np = data['window_info'].astype(np.int64)
            out['window_info'] = torch.from_numpy(window_info_np)
