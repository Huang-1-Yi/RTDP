...
from diffusion_policy.common.sampler_rt import SequenceSamplerRT
...
class RobomimicReplayImageDatasetRT(BaseImageDataset):
    def __init__(self,
        ...
        # ========= 构建窗口级采样器与样本顺序（训练集） =========
        self._build_window_sampler_and_order(episode_mask=self.train_mask)
    ...
    def get_validation_dataset(self):
        ...
        # ========= 为验证集构建窗口级采样器与样本顺序 =========
        val_set._build_window_sampler_and_order(episode_mask=val_set.train_mask)
        return val_set
    ...
    def __len__(self):
        ...
        我们将样本顺序编排为：(固定窗口步 w，遍历所有被选 episode)，
        因此 DataLoader(batch_size=B, shuffle=False) 会把相邻的 B 个样本堆叠成
        [B, H, ...]，即“同一窗口步的 B 个 episode”。
        """
        return int(len(getattr(self, '_window_global_indices', [])))

    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        """返回一个窗口样本（单 episode）。

        注意：为配合按窗口步对齐的 batching，请在 DataLoader 中设置 shuffle=False。
        """
        threadpool_limits(1)
        # 将逻辑索引映射到 sampler 的全局窗口索引
        gidx = int(self._window_global_indices[idx])
        sample = self.sampler_rt.sample_sequence(gidx)

        # 组织观测
        obs_np = {}
        for key in self.rgb_keys:
            if key in sample:
                obs_np[key] = np.moveaxis(sample[key], -1, 1).astype(np.float32) / 255.
        for key in self.lowdim_keys:
            if key in sample:
                obs_np[key] = sample[key].astype(np.float32)
        # 附带窗口信息（int64, [H,5]）放到顶层，避免进入 obs 正则化与编码路径
        window_info_np = sample['window_info'].astype(np.int64)

        # 动作
        action_np = sample['action'].astype(np.float32)

        return {
            'obs': dict_apply(obs_np, torch.from_numpy),
            'action': torch.from_numpy(action_np),
            'window_info': torch.from_numpy(window_info_np)
        }

    # ========= 辅助：构建窗口级采样器与样本顺序 =========
    def _build_window_sampler_and_order(self, episode_mask: np.ndarray):
        """根据给定的 episode_mask 构建窗口采样器，并生成“按窗口步交错的”索引顺序。"""
        obs_keys_for_sampler = list(self.rgb_keys) + list(self.lowdim_keys)
        self.sampler_rt = SequenceSamplerRT(
            replay_buffer=self.replay_buffer,
            sequence_length=int(self.horizon),
            pad_before=int(self.pad_before),
            pad_after=int(self.pad_after),
            keys=list(self.replay_buffer.keys()),
            episode_mask=episode_mask,
            obs_keys=obs_keys_for_sampler,
            n_obs_steps=int(self.n_obs_steps) if self.n_obs_steps is not None else None,
            action_key='action',
            equalize_windows_per_episode=False
        )
        print("窗口采样器已构建。equalize_windows_per_episode=False")
        sel_ids = self.sampler_rt.selected_episode_ids()
        counts_by_sel = getattr(self.sampler_rt, '_concat_counts_by_sel', None)
        if counts_by_sel is None or len(counts_by_sel) == 0:
            self._num_window_steps = 0
            self._num_episodes_in_split = 0
            self._window_global_indices = np.zeros((0,), dtype=np.int64)
            self._is_window_level = True
            return
        max_count = int(counts_by_sel[0])
        n_ep = len(sel_ids)
        self._num_window_steps = max_count
        self._num_episodes_in_split = n_ep
        # sampler_rt.indices 的布局：按 episode 依次排列，每个 episode 有 max_count 个窗口
        # 因此 episode i 的窗口 w 的全局索引为 i*max_count + w
        order = []
        for w in range(max_count):
            for i in range(n_ep):
                order.append(i * max_count + w)
        self._window_global_indices = np.asarray(order, dtype=np.int64)
        # 声明当前数据集为窗口级
        self._is_window_level = True
